...
public class GeneAnnotator {
    ...
    public void enrichmentTestGeneSet(String genesetDBFile, Map<String, double[]> genePValueMap, double genePValueCutoff,
            int minSize, int maxSize, String geneSumOutFile) throws Exception {
        GeneSetExplorer genesetExplor = new GeneSetExplorer(genesetDBFile);
        genesetExplor.loadGSEAGeneSets(minSize, maxSize);
        Map<String, GeneSet> dbPathwaySet = genesetExplor.getGeneSetSet();
        ...
        for (Map.Entry<String, GeneSet> mPath : dbPathwaySet.entrySet()) {
            ...
            for (String pGeneSymb : geneSymbolWeightMap) {
                ...
                availableGWASGeneList.add(new String[]{pGeneSymb, String.valueOf(p1[1])});
                zValueListInGeneSet.add(p1[1]);
            ...
            for (Map.Entry<String, double[]> mGene : genePValueMap.entrySet()) {
                if (!pGenes.contains(mGene.getKey())) {
                    ...
                    zValueListOutPathway.add(mGene.getValue()[1]);
                }
            }
    ...
    public void geneMutationRateTest(Genome genome, Map<String, Double> geneLengths, Map<String, Map<String, Integer>> cosmicGeneMut, Map<String, double[]> geneScores,
        ...
        for (Map.Entry<String, double[]> geneP : geneMutRegPValueAllVar.entrySet()) {
            if (geneP.getValue()[1] <= adjGenePValueCutoff) {
