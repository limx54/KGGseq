 ...
 * @author mxli
 */
public class VCFParser implements Constants {

    private static final Logger LOG = Logger.getLogger(VCFParser.class);
    ...
    public int readVariantsInFileOnlyFastToken(int threadNum, Genome orgGenome, File dataFile, final Genome targGenome,
        ...
        if (dataFile.exists()) {
            try {
                ...
                br = new BufferedReader(Channels.newReader(Channels.newChannel(new GZIPInputStream(Channels.newInputStream(new RandomAccessFile(dataFile, "r").getChannel()))), Charset.forName("UTF-8").newDecoder(), 1024 * 1024 * 100));
            } catch (ZipException ex) {
                // br = new BufferedReader(new InputStreamReader(new FileInputStream(dataFile)));
                br = new BufferedReader(Channels.newReader(Channels.newChannel(Channels.newInputStream(new RandomAccessFile(dataFile, "r").getChannel())), Charset.forName("UTF-8").newDecoder(), 1024 * 1024 * 100));
            }
...
//System.out.println(currentLine); 
            ...
            for (int s = 0; s < maxThreadNum; s++) {
                parsTaskArray[s] = new VCFParseTask(s);
